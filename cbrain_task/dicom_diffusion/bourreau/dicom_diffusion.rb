...
class CbrainTask::DicomDiffusion < ClusterTask
  ...
  def cluster_commands #:nodoc:
    ...
    inputs.each do |userfile|
      ...
      cmdl_params =  "#{indir}/#{userfile.get_nii_basename.bash_escape} --out_strat absolute --out_dir #{outputdir}/#{userfile.id}/ --tag #{tag} --nlmeans.sigma #{sigma} --csd.frf #{frf} --csd.b0_threshold #{b0_threshold} --csa.b0_threshold #{b0_threshold} --dti.b0_threshold #{b0_threshold}"
